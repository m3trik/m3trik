...
| `check_public_hygiene.py [repo…]` | a client identifier in anything `git add -A` would publish from the public repos (paths + every text file, CHANGELOGs included) | The denylist is PRIVATE -- `<workspace>/.claude/public_hygiene.txt`, outside every repo; absent → SKIP. `push.ps1` runs it before a release. Never copy a pattern into a repo, test, commit or PR body; fix a false positive in the denylist. |
...
| `sync_shadow_shaders.py [--check]` | drift between `pythontk/geo_utils/shadow_horizon.glsl` and its two generated mirrors (`shadow_rig.js`, `unitytk` `ShadowPlaneHorizon.hlsl`) | **Marker**-spliced, not whole-file: each mirror keeps its own uniforms + `SH_Fetch` hook outside the region, and a missing marker pair is refused. Maya/Blender assemble instead, via `ShadowHorizon.shader_source(language)`. `pythontk/test/test_sync_shadow_shaders.py` pins it. |
