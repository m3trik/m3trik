...
| `check_temp_artifacts.py` | production source allocating temp files/dirs outside `ptk.TempArtifacts` (`mkstemp`/`mkdtemp`/`NamedTemporaryFile(delete=False)`) | Self-cleaning forms and bare `gettempdir()` pass; tests exempt. Exceptions go in its `ALLOWLIST` with a reason — a growing list means the primitive lacks a capability. |
