...
def emit_inline_index(pkg: PackageData) -> str:
    """Compact module-level index suitable for embedding in CLAUDE.md."""
    lines: list[str] = []
    lines.append(
        f"_Auto-generated by `m3trik/scripts/generate_api_registry.py`. "
        f"Full registry: [API_REGISTRY.md](API_REGISTRY.md). "
        f"Last refresh: {pkg.generated_at}._"
    )
    lines.append("")
    lines.append("| Module | Summary |")
    lines.append("|---|---|")
    for mod in pkg.modules:
        summary = mod.summary or "_(no module docstring)_"
        # Markdown table cells: pipes need escaping
        summary = summary.replace("|", "\\|")
        lines.append(f"| `{mod.relpath}` | {summary} |")
    return "\n".join(lines)
...
def emit_shadow_report(packages: list[PackageData]) -> str:
    ...
    for name in sorted(cross_pkg):
        occs = cross_pkg[name]
        lines.append(f"## `{name}` ({len(occs)} definitions)")
        lines.append("")
        for pkg_name, relpath, qualname, line in occs:
            lines.append(f"- `{pkg_name}` — [`{qualname}`]({pkg_name}/{relpath}#L{line})")
        lines.append("")
    return "\n".join(lines).rstrip() + "\n"
