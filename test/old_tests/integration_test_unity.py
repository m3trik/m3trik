...
try:
    ...
    print(
        "Please run this script within the Maya environment with 'mayatk' accessible."
    )
    sys.exit(1)
...
def setup_test_scene():
    ...
    RenderOpacity.create([sphere_attr], mode="attribute")
    ...
    # Verify
    has_keys = (
        pm.keyframe(sphere_attr, attr="opacity", query=True, keyframeCount=True) > 0
    )
    ...
    RenderOpacity.create([sphere_mat_1, sphere_mat_2], mode="material")
    ...
    print(
        "   [Attribute Mode Object]: Add 'RenderOpacityController.cs' component (or ensure FadeCurveFixer runs)."
    )
    print(
        "   [Material Mode Objects]: Should fade independently. Check Materials in Inspector."
    )
    print("-" * 50)
